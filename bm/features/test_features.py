...
def test_builder(start: float, stop: float, events_df) -> None:
    ...
    features = [x for x in sorted(FeaturesBuilder._FEATURE_CLASSES) if (not x.startswith(
        "Xlm") and not ("Wav2Vec" in x) and not ("Hubert" in x) and not ("XLSR" in x)
        and not ("WavLM" in x))]
    features_params = {
...
@pytest.mark.parametrize("cls", [audio.HubertConvolution, audio.HubertTransformer])
def test_hubert(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
...
@pytest.mark.parametrize("cls", [audio.HubertConvolutionLarge, audio.HubertTransformerLarge])
def test_hubert_large(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
...
@pytest.mark.parametrize("cls", [audio.HubertConvolutionXLarge, audio.HubertTransformerXLarge])
def test_hubert_xlarge(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
...
@pytest.mark.parametrize("cls", [audio.XLSRConvolution1b, audio.XLSRTransformer1b])
def test_xlsr_1b(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
...
@pytest.mark.parametrize("cls", [audio.XLSRConvolution2b, audio.XLSRTransformer2b])
def test_xlsr_2b(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
...
@pytest.mark.parametrize("cls", [audio.WavLMBasePlusConvolution, audio.WavLMBasePlusTransformer])
def test_wavlm_base_plus(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
...
@pytest.mark.parametrize("cls", [audio.WavLMBaseConvolution, audio.WavLMBaseTransformer])
def test_wavlm_base(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
...
@pytest.mark.parametrize("cls", [audio.WavLMLargeConvolution, audio.WavLMLargeTransformer])
def test_wavlm_large(cls, tmp_path: Path) -> None:
    if os.environ.get("CIRCLECI", ""):
        pytest.skip("Models do not run in the CI")
    wavpath = str(Path(__file__).parent.parent / "mockdata" / "one_two.wav")
    event = events.Sound(start=1, duration=1, filepath=wavpath, modality=None, language=None)
